...
function View() {
    ...
    this.create_board = (image_array) => {
        ...
        for (let i = 0; i < random_images.length; i++) {
            ...
            $('<div>').addClass('back').prepend('<img src="images/rose.jpg"/>').appendTo('#card' + i);
        } //end for loop
...
function Controller(images, sounds) {
    ...
    this.toggle_disabled_reset = () => {
        ...
        if (!this.reset_lock) {
            ...
            reset_button.css('background-color', 'pink');
        } else {
